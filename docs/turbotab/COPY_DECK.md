...
| State | Trigger | Copy | Source |
|---|---|---|---|
| findings stack · more than the bound was found | `attention.stack` collapses anything — the affordance's own sentence, counted and typed by severity | {n} more — 3 warnings, 4 cautions | `turbotab/attention.py` |
| findings stack · the remainder, typed by stream | the line beneath the affordance; the profile speaks about the table and a pack speaks about the field | 2 from the clinical lens · 5 about this table | `turbotab/attention.py` |
| findings stack · the affordance's stated effect | hover and screen reader, per §05.1 — the control states what it will do, and never as a bare verb | Adds {n} more findings to this list. They ranked below the top {bound}; nothing is out of the record either way. | `turbotab/attention.py` |
| findings stack · nothing was collapsed | `attention.stack` collapses nothing. §09's recorded-absence rule: without this a reader cannot tell *this is everything* from *this is the top few*, which is two claims rendering as one | All {n} shown.  ·  Nothing stood out in the profile or under the lens. | `turbotab/attention.py` |
| findings stack · a card was promoted into a cleared slot | a dismissal or deferral frees budget and the next collapsed finding is pushed. §09's recorded-absence rule from the other side: an object appearing without explanation is as unexplained as one vanishing without it | Moved up when you dismissed a card above.  ·  …deferred…  ·  …cleared… (when both kinds occurred) | `turbotab/attention.py` |
| deferral · the attribution it comes back with | a deferred noticing renders at the step `ml.router.PACK_DEFER` sent it to. `PRODUCT_VISION.md` §04: *pre-checked and attributed*, and the attribution is this sentence | You set this aside at Explore. {Step} is the step that can act on it. | `turbotab/attention.py` |
| deferral · deferred with no destination | a finding deferred while carrying no `defer_target`. Reported rather than filed under a step nobody chose — `GUIDED-153` was exactly that silence | This was set aside and nothing recorded where it comes back, so it cannot be shown at a step. That is a defect rather than a state. | `turbotab/attention.py` |
| impossible entries · set to missing | the repair executes row-locally on the working table. The sentence is the SERVER's now — it used to be composed in the page and asserted a repair that never ran | {n} entries of `{column}` outside the impossibility band of {low}–{high} {unit} were set to missing. | `turbotab/project.py` |
| impossible entries · kept as recorded | its own decision kind, because both buttons used to post `note` with the same subject and only the prose differed | {n} entries of `{column}` outside the impossibility band were kept as recorded. | `turbotab/project.py` |
| prevalence · the subject is not a nutrient | `prevalence_of_inadequacy` is asked about a column the dietary pack does not recognize. Runs BEFORE the reference and basis branches: *this is not a nutrient* dominates *the RDA is the wrong reference for it* | `{name}` is not a nutrient this pack recognizes — it is a respondent identifier. … this app holds none for `{name}`. Answering anyway would put a SETTLED nutritional claim on whatever column was selected. | `turbotab/nutrition.py` |
| any control · the server refused the press | `setErr` or `showRefusal` with a control that carries a `data-ac` slot. The canonical `#upErr` still carries it — it is the visible sink DURING upload, and `#sub-upload` is `display:none` from the first render after | (the server's own reason, quoted — never composed here) | `turbotab/web/index.html` |
