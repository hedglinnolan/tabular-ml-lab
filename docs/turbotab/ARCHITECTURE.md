...
- **Persist decisions, regenerate derivatives.** *(Corrected — I previously wrote "nothing is
  written to disk," which inverted the contract and created a false dilemma. See below.)*
  `utils/session_manager.py` already saves a session as a zip of `manifest.json`, `config.json`,
  `widget_state.json`, `coaching.json` (the ledger), `provenance.json`, **`lockbox.json` (the
  sealed test-set labels)**, `cohort.json`, `cohort_runs.json`, `fe_recipe.json` and
  `coach_probe.json`. It refuses pickles outright.
  `_NEVER_PERSIST` is a **drop-list**, not a prohibition: trained artifacts, fitted pipelines,
  splits, derived metrics, profiles and export artifacts are dropped on save because they are
  *unsafe to pickle or cheap to regenerate* — the comments say so ("regenerated by re-running
  Page 06", "regenerated from raw_data + split_config").
  So the real invariant is: **a saved project carries decisions and inputs, never derived
  artifacts and never fitted objects.** A job queue does not threaten this; a job queue that
  cached fitted estimators to disk would.
...
| Persistence | `session_manager` is Streamlit-bound but the **format is already right** — decisions and inputs, no pickles, lockbox labels included. Port the archive schema; do not redesign it. | low |
